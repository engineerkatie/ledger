...
BasicVmEngine::ExecutableLoader::ExecutableLoader(VM &vm, Executable *exe)
  : vm_(vm)
  , exe_(exe)
{
  vm_ . LoadExecutable(exe_);
}
BasicVmEngine::ExecutableLoader::~ExecutableLoader()
{
  vm_ . UnloadExecutable();
}

ExecutionResult BasicVmEngine::RunDirectConversion(Name const &       execName,
...
if (!HasExecutable(execName))
  ...
  try
  {
    ExecutableLoader loader(vm, exec.get());
    ...
    for (std::size_t parameter_number = 0; parameter_number < numParameters; ++parameter_number)
    {
      ...
      //if (Convertable(params[parameter_number], type_id))
      //{
      //  parameterPack.AddSingle(Convert(params[parameter_number], type_id));
      //  continue;
      //}

      std::string working_type_name = vm . GetTypeName(type_id);

      VarConverter          vc;
      auto foo = vc.Convert(params[parameter_number], working_type_name, vm);
      parameterPack.AddSingle(foo);
    }
  }
